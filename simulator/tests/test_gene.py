...
class GeneCreationTests(unittest.TestCase): 
    def test_IO_exception(self):
        self.assertRaises(Gene("simulator/tests/genes/basic_gene_nonexist.json"))
    
    def test_bad_json_exception(self):
        self.assertRaises(Gene("simulator/tests/testing_data/genes/impropper_format_gene.json"))

...
class Variant: 
    ...
    def __init__(self, var_template):
        ...
        if self.zygosity not in ['HOMOZYGOUS', 'HETEROZYGOUS', 'HEMIZYGOUS']:
            raise ValueError('must have a zygosity within this list HOMOZYGOUS, HETEROZYGOUS, HEMIZYGOUS')
    ...
    def parse_region(transcript, var_region):
        ...
        if end > Chrom().chrom_size(session, chrom):
            raise ValueError("invalid end position, greater than chromosome length")  
        if chrom != transcript.chrom:
            raise ValueError("custom location is not on the same chromosome as the transcript")
